...
TableModel::TableModel(QObject *parent) : QAbstractTableModel(parent)
{
    ...
    connect(uploader, SIGNAL(authFinished(bool,QString)), this, SLOT(authFinished(bool,QString)));
...
void TableModel::startUploading()
{
    if (list.size()>0)
    {
        ...
        {
            emit statusChanged(tr("Authentification"));
            uploader->auth(settings->region, settings->email, settings->password);
        }
        else
        {
            list[currentIndex].status = StatusInProgress;
            emit dataChanged(this->index(currentIndex, 2), this->index(currentIndex, 2));
            uploader->upload(settings->region, list[currentIndex].filename, settings->passForDown, settings->comment);
        }
    }
}
...
void TableModel::authFinished(bool success, const QString &errortext)
{
    if (success)
    {
        emit statusChanged(tr("Authentification successful"));
        list[currentIndex].status = StatusInProgress;
        emit dataChanged(this->index(currentIndex, 2), this->index(currentIndex, 2));
        uploader->upload(settings->region, list[currentIndex].filename, settings->passForDown, settings->comment);
        //emit allUploadsStarted();
    ...
    {
        emit statusChanged(tr("Authentification failed: %1").arg(errortext));
